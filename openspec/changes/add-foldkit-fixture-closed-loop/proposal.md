...
- Propagate FoldKit-driven fixture events through the real read-model projection, Effect's experimental `Reactivity` invalidation keys, and Effect's experimental `AtomRegistry` before refreshed `WorkbenchSnapshot` state reaches FoldKit.
...
- Add tests that replay the fake route twice and verify stable event order, stable `DecisionPacket` output, refreshed `WorkbenchSnapshot` version, and updated FoldKit render state.
...
- Affects `packages/attune-foldkit` and `packages/attuned-discovery`.
...
- Adds tests around route replay, snapshot freshness, and FoldKit rendering.
