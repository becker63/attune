...
describe("cocoindex-effect", () => {
  it("declares CocoIndex effect recipes from the package barrel", () => {
    ...
    expect(records.map((record) => record.recipeId)).toEqual([
      "cocoindex-effect.source-surface",
      "cocoindex-effect.client-contract",
      "cocoindex-effect.fixture-client-layer",
      "cocoindex-effect.error-taxonomy",
      "cocoindex-effect.generation-cli-invocation",
      "cocoindex-effect.mcp-tool-generation",
      "cocoindex-effect.test-suite",
    ...
    expect(records.some((record) =>
      record.sourcePath === "packages/attune/cocoindex-effect/src/recipes.ts"
    )).toBe(false)
  ...
  it("keeps live MCP schema and registry authored while generation writes cache artifacts", () => {
    const schemaPath = "packages/attune/cocoindex-effect/src/cocoindex/mcp-schema.ts"
    const schemaSource = readFileSync(new URL(`../../../../${schemaPath}`, import.meta.url), "utf8")

    expect(schemaSource).toContain("CocoIndexCodeMcpSchemaSnapshot")
    ...
    expect(RecipeObservationView.generatedArtifactFreshness({
      ...
      artifactPath: ".attune/cache/generated/cocoindex-effect/cocoindex-code-mcp.ts",
      fresh: true,
