...
The generated files are derived from Joern {{JOERN_VERSION}} and codepropertygraph {{CPG_VERSION}}, both sourced from `flake.nix`.
...
When the layer is acquired, `joern-effect` resolves Joern from `JOERN_BINARY` or `joern` on `PATH`, chooses a free localhost port, starts Joern server mode, waits for readiness, imports the repository, and provides a typed query client. When the Effect scope exits, the Joern process is stopped. Server readiness defaults to 120 seconds for cold WSL/Nix starts and can be overridden with `JOERN_READY_TIMEOUT_MS`.
...
- Joern: {{JOERN_VERSION}}
- codepropertygraph: {{CPG_VERSION}}
- Node.js: {{NODE_PACKAGE}}
- JDK: {{JDK_PACKAGE}}
- pnpm package manager: {{PNPM_PACKAGE_MANAGER}}
...
The checked-in schema snapshot is `schema/joern-cpg-schema.{{CPG_VERSION}}.json`. It is extracted by compiling `src/internal/generation/ExtractCpgSchema.java` against the Nix-built Joern jars and reading Joern's real `GraphSchema` metadata.
