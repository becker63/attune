...
import { booleanField, nodeField } from "./_ast-record.js";
...
const isDirectlyNestedGen = (node: ESTree.Node): boolean => {
  ...
  if (booleanField(yieldExpr, "delegate") !== true) return false;
      ...
      Visitor.tracked("CallExpression", (node) => AST.isCallOf(node, "Effect", "gen"), depth),
      {
        CallExpression: (node) =>
          Effect.flatMap(Ref.get(depth), (d) => {
            ...
            if (!AST.isCallOf(node, "Effect", "gen")) return Effect.void;
            if (!isDirectlyNestedGen(node)) return Effect.void;
            return ctx.report(
              Diagnostic.make({
                node,
