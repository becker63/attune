...
const generatedDir = fileURLToPath(
  new URL("../src/pure/generated/", import.meta.url),
);
...
const forbiddenImports = [
  {
    pattern:
      /from\s+["']node:(?:child_process|fs|fs\/promises|process|http|https|net|dgram)["']/u,
    reason: "generated public surfaces must not import Node runtime IO modules",
  },
  {
    pattern: /from\s+["']\.\.\/\.\.\/edge\/runtime/u,
    reason:
      "generated public surfaces must not import Joern runtime interpreters",
  },
] as const;
...
const forbiddenRuntimeTokens = [
  ...
  },
  {
    ...
    reason:
      "generated public surfaces must not execute Effect programs during construction",
  },
  {
    pattern: /\bprocess\.(?:env|cwd|exit|stdout|stderr)\b/u,
    reason:
      "generated public surfaces must not inspect or mutate process state",
  },
  {
    pattern: /\bfetch\s*\(/u,
...
describe("generated Joern public surface", () => {
  it("stays descriptive and free of runtime IO during construction", () => {
    expect(
      generatedFiles.map((file) => file.replace(generatedDir, "")),
    ).toEqual(["cpg.ts", "nodes.ts", "prop.ts", "schema.ts"]);
    ...
    for (const file of generatedFiles) {
      ...
      for (const rule of forbiddenImports) {
        expect(source, `${relativeFile}: ${rule.reason}`).not.toMatch(
          rule.pattern,
        );
      }
