...
The API documentation compiler SHALL no longer inspect, drift-check, or
publish experiment reports when no experiment publication bundle is selected.

**Reason**: Python owns experiment generation and drift checking. Retaining a
TypeScript-side experiment check in the disabled-publication case would
recreate a second authority and an unused adapter boundary.

**Surviving contract**: The `grounded-experiment-reports` capability governs
Python publication bundles. If a later change explicitly enables independent
experiment publication, TypeScript may verify the selected immutable bundle
without recalculating or rewriting its facts.

### Requirement: Static experiment discovery

The API documentation build SHALL no longer discover experiment pages, create
experiment search records, or mix experiment navigation into the API
document.

**Reason**: Experiment facts and navigation are independent of the closed
type-reference document. Python already owns the approved publication bundle.

**Surviving contract**: Approved generated Markdown may reuse the generic
sanitized MDAST-to-HAST publication tail under a separate static namespace as
specified by `grounded-experiment-reports`. This change selects no approved
bundle and therefore implements no adapter.

### Requirement: Versioned API manifest

The documentation product SHALL no longer publish or validate an API manifest
or manifest schema.

**Reason**: Ordinary transient MDAST is the only intermediate tree, HAST is a
disposable lowering, and `index.html` is the only API publication. A versioned
manifest would recreate the parallel identity and projection system this
change removes.

**Surviving contract**: Source annotations, TSDoc, resolved compiler facts,
VFile messages, immutable links, and focused HTML contracts directly govern
the one document. No consumer may depend on an intermediate JSON artifact.
