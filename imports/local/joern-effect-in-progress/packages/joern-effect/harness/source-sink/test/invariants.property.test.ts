...
describe("attune bridge decode property pressure", () => {
  it("bridge decodes generated valid rows and rejects malformed rows structurally", async () => {
    await checkAttuneProperty({
      arbitrary: validRow,
      invariantId: "unknown-is-owned-by-bridge",
      numRuns: 60,
      phase: "bridge",
      propertyId: "bridge-decode-valid-invalid-rows",
      target: "joern-effect:property:bridge",
      predicate: async (row) => {
...
describe.skipIf(!Boolean(readEnv(EnvVars.JoernBinary) ?? readEnv(EnvVars.JoernHome)))(
  "attune loop Joern-gated property pressure",
  () => {
    it("joern runtime smoke stays Effect-scoped when enabled", async () => {
      await checkAttuneProperty({
        ...
        predicate: async () => {
          const layer = Joern.layer({
            repoPath: readEnvOr(EnvVars.JoernEffectTestTmpdir, "/dev/shm"),
