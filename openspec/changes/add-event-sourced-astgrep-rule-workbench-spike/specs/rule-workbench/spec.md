...
- **THEN** the Rule Workbench shall show the candidate intent, both examples, the structural proxy, the native rule, compact measurement status, compact findings handoff, known limits, and route access to detailed lineage
...
- **THEN** the system shall render fixture events, projection state, highlighted examples, highlighted deterministic rule, compact measurement, compact findings handoff, and export preview path in the Workbench
...
### Requirement: Human-readable lineage page

The system SHALL expose readable lineage derived from domain events on a dedicated Lineage page rather than as a bottom bar on the default Workbench.

#### Scenario: Render lineage from events

- **WHEN** candidate generation, measurement, labeling, revision, and promotion events exist
- **THEN** the Lineage page shall show a timeline explaining what happened, what changed, and why the candidate can be trusted or rejected
