...
const ClosedWorldOptions = Schema.Struct({
...
const OptionalClosedWorldOptions = Schema.UndefinedOr(ClosedWorldOptions);
...
const implicitGlobalObjects = new Set(["globalThis", "window", "document"]);
...
const runtimeBoundaryFilename = (filename: string): boolean =>
  /\.test\.tsx?$/.test(filename) ||
  /\/scripts\//.test(filename) ||
  /\/examples\/raw-cpgql\.ts$/.test(filename);

const generatedDslTypeBoundary = (filename: string): boolean =>
  /\/src\/pure\/builder\/(?:select|traversal)\.ts$/.test(filename);
...
const callRoot = (node: ESTree.Node | null | undefined): ESTree.Node | undefined =>
  node?.type === "CallExpression" ? callRoot(node.callee) : node ?? undefined;

const isEffectModelClass = (node: ESTree.Node): boolean => {
  const root = callRoot(node);
  return (
    isMemberCallTarget(root, "Schema", new Set(["Class", "TaggedClass", "TaggedErrorClass"])) ||
    isMemberCallTarget(root, "Data", new Set(["TaggedClass", "TaggedError"])) ||
    isMemberCallTarget(root, "Context", new Set(["Tag"]))
  );
};
...
const isConstTypeReference = (node: unknown): boolean =>
  isAstRecord(node) &&
...
const isConstKeyword = (node: unknown): boolean =>
  isAstRecord(node) && node["type"] === "TSConstKeyword";
...
export const onlyAllowedImports = Rule.define({
  ...
  options: OptionalClosedWorldOptions,
  meta: Rule.meta({
    type: "problem",
...
export const noInterfaces = Rule.define({
  ...
  create: function* () {
    const ctx = yield* RuleContext;
    if (generatedDslTypeBoundary(ctx.filename)) return {};
    return {
...
export const noClasses = Rule.define({
  ...
  create: function* () {
    ...
    const report = (node: ESTree.Node): Effect.Effect<void> => {
      const superClass = isAstRecord(node) ? node["superClass"] : undefined;
      return isAstNode(superClass) && isEffectModelClass(superClass)
        ? Effect.void
...
export const noTypeAssertions = Rule.define({
  ...
  create: function* () {
    ...
    const reportAsExpression = (node: ESTree.Node): Effect.Effect<void> =>
      "typeAnnotation" in node &&
      (isConstTypeReference(node.typeAnnotation) ||
        isConstKeyword(node.typeAnnotation))
        ? Effect.void
        : report(node);
    return {
      TSAsExpression: reportAsExpression,
...
export const noRuntimeExecution = Rule.define({
  ...
  create: function* () {
    const ctx = yield* RuleContext;
    if (runtimeBoundaryFilename(ctx.filename)) return {};
