...
describe("RecipeKernel", () => {
  ...
  it("keeps the framework recipe receipt SQL spine explicit", () => {
    ...
    expect(frameworkRecipeReceiptSafeQlConfig().checkedStatements).toEqual(expect.arrayContaining([
      ...
      "SELECT * FROM framework_event.recipe_observation WHERE payload->>'benchmarkRunId' = $1 AND payload->>'packetId' = $2 ORDER BY observed_at ASC, observation_id ASC",
    ]))
    ...
    expect(frameworkRecipeReceiptSqlValidationStatements().map((statement) => statement.name))
      .toEqual(expect.arrayContaining([
        ...
        "benchmark-observations-by-run",
        "benchmark-packet-observations-by-packet",
        "benchmark-packet-observations-by-rule",
        "benchmark-packet-observations-by-profile",
        "benchmark-packet-validation-by-status",
        "benchmark-packet-report-projection-inputs",
        "benchmark-report-projection-inputs",
        "benchmark-loop-observations-by-kind",
        "benchmark-target-status-latest",
        "benchmark-target-status-by-goal-status",
        "benchmark-exact-packet-targets",
        "benchmark-registration-observations",
        "benchmark-holdout-commitments",
        "benchmark-holdout-evaluations",
        "benchmark-negative-controls",
        "benchmark-all-in-cost-ledger",
        "benchmark-corrected-scorecard-inputs",
        "benchmark-audit-inputs",
      ]))
  ...
  it("validates measurement observation SQL projection statements", () => {
    ...
    expect(names).toEqual(expect.arrayContaining([
      ...
      "measurement-report-projection-inputs",
      "benchmark-observations-by-run",
      "benchmark-observations-by-arm",
      "benchmark-observations-by-measurement-session",
      "benchmark-codex-thread-by-run",
      "benchmark-final-judge-by-status",
      "benchmark-packet-observations-by-packet",
      "benchmark-packet-observations-by-rule",
      "benchmark-packet-observations-by-profile",
      "benchmark-packet-validation-by-status",
