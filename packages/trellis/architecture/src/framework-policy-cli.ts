...
const activePackageSourceRootPattern =
  /^(?<root>packages\/(?:(?:attune|canopy|tend|trellis)\/[^/]+|[^/]+))\/src\//u
const activePackageProjectFilePattern =
...
const oneFileSurfaceCompletedRoots = new Set([
  ...
  "packages/attune/cocoindex-effect",
...
const temporaryAtomReactivityDebt = [
  {
    packageRoot: "packages/attune/discovery",
  ...
  {
    packageRoot: "packages/attune/joern-effect",
...
function checkPackageLocalAttuneSurface(
  ...
  return [finalRatchetDiagnostic(
    ...
    [
      ...
      `Run nx run ${projectNameForRoot(packageRoot, filesByPath)}:repair or workspace:repair when the repair target supports this root.`,
    ].join(" "),
    severity,
  )]
}
...
function checkPackageLocalAttuneCompanionImports(
  ...
  if (file.path.startsWith("packages/trellis/architecture/src/generated/")) return []
  ...
  for (const match of file.content.matchAll(packageLocalAttuneCompanionImportPattern)) {
    ...
    diagnostics.push(finalRatchetDiagnostic(
      ...
      [
        ...
        replacementText,
        `Run nx run ${projectName}:repair or workspace:repair when the repair target supports this root.`,
      ].join(" "),
      severity,
    ))
  }
...
function findActivePackageRoots(files: readonly WorkspaceFile[]): readonly string[] {
  ...
  for (const file of files) {
    const match = activePackageProjectFilePattern.exec(file.path)
    const owner = match?.groups?.owner
    const name = match?.groups?.name
    const legacyName = match?.groups?.legacyName
    if (owner !== undefined && name !== undefined) roots.add(`packages/${owner}/${name}`)
    if (legacyName !== undefined) roots.add(`packages/${legacyName}`)
  }
...
function findAuthoredAttuneRoots(files: readonly WorkspaceFile[]): readonly string[] {
  ...
  for (const file of files) {
    const match = authoredAttunePackageFilePattern.exec(file.path)
...
function projectNameFromPackageRoot(packageRoot: string): string {
  const activePackageRoot = activePackageRootPattern.exec(packageRoot)
  if (activePackageRoot?.groups?.name !== undefined) return activePackageRoot.groups.name
  if (activePackageRoot?.groups?.legacyName !== undefined) return activePackageRoot.groups.legacyName
...
function isPrimaryProgramIndexPath(filePath: string): boolean {
  return (
    ...
    filePath === "packages/trellis/runtime/src/ProgramIndexProjection.ts" ||
    filePath === "packages/trellis/nx/src/ProgramGraphIndex.ts"
  )
}
...
function isTemporaryFrameworkPolicyWaiver(filePath: string, importSource: string): boolean {
  return (
    filePath === "packages/trellis/architecture/src/framework-import-boundary.ts" ||
    (
      filePath === "packages/attune/discovery/src/memory/schema.ts" &&
