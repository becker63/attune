...
- **WHEN** the Attune app renders code, metrics, status, findings, lineage, or export surfaces
...
### Requirement: Peer example panes

The Workbench SHALL render positive and negative examples as independent peer artifact panes rather than inside a shared examples parent.

#### Scenario: Render peer examples

- **WHEN** a candidate has both required examples
- **THEN** the Workbench shall render separate `Looks like` and `Does not look like` panes
...
- **THEN** the deterministic rule pane shall appear to the right of the example panes and span the main content height
...
### Requirement: Lineage belongs on a dedicated page

The Workbench SHALL NOT render the full provenance timeline by default; detailed lineage belongs on the Lineage page.

#### Scenario: Omit Workbench lineage bar

- **WHEN** candidate generation, measurement, revision, review, promotion, or export events exist
- **THEN** the default Workbench route shall not render a bottom provenance or lineage timeline

#### Scenario: Preserve lineage route

- **WHEN** the user opens the Lineage route for a selected candidate
- **THEN** the app shall be able to render human-readable lineage derived from events rather than raw event payload labels
...
### Requirement: FoldKit-native iconography

The system SHALL render product icons as FoldKit-native inline SVG nodes using a curated line-icon helper.

#### Scenario: Render shell and Workbench icons

- **WHEN** the sidebar, Workbench status strip, examples, findings summary, deterministic rule pane, lineage page, or export surfaces render
- **THEN** icons shall render as FoldKit `Html` SVG nodes styled with `currentColor`

#### Scenario: Avoid icon font runtime

- **WHEN** Attune renders the default Workbench UI
- **THEN** it shall not depend on an icon font, React icon component runtime, or web-component icon runtime for core product icons

#### Scenario: Preserve icon accessibility

- **WHEN** an icon is decorative
- **THEN** it shall be hidden from assistive technology
- **AND WHEN** an icon carries meaning by itself
- **THEN** it shall have an accessible label

### Requirement: Viewport-contained Workbench layout

The Workbench SHALL fit the desktop application shell inside the viewport and use local overflow regions for dense evidence.

#### Scenario: Fit desktop Workbench in viewport

- **WHEN** the Workbench renders on a desktop-size viewport
- **THEN** the sidebar, header, compact status, artifact grid, and compact findings handoff shall fit within the application shell without requiring document-level scrolling

#### Scenario: Scroll local evidence regions

- **WHEN** sidebar patterns or code panes exceed their available space
- **THEN** those regions shall scroll locally rather than pushing critical Workbench controls outside the viewport

#### Scenario: Preserve mobile reading flow

- **WHEN** the Workbench renders on a narrow viewport
- **THEN** the layout may collapse to a single-column document flow while preserving the same information hierarchy

### Requirement: Expandable and resizable code panes

The Workbench SHALL let engineers expand each code artifact pane and resize code panes with pointer interaction.

#### Scenario: Expand code pane

- **WHEN** the user activates the expand control for `Looks like`, `Does not look like`, or `Deterministic rule`
- **THEN** that code pane shall become the focused artifact in the Workbench artifact area
- **AND** the expanded state shall be represented in FoldKit model state

#### Scenario: Collapse code pane

- **WHEN** the user activates the collapse control for an expanded code pane
- **THEN** the Workbench shall return to the normal artifact layout showing examples and deterministic rule together

#### Scenario: Resize code pane

- **WHEN** an example pane or deterministic rule pane is visible
- **THEN** the user shall be able to resize that pane with mouse or pointer interaction without mutating product truth
...
- **THEN** the test shall verify sidebar pattern selection, peer example panes, right-side deterministic rule pane, compact findings handoff, absence of bottom provenance timeline, and absence of standalone measurement panel
