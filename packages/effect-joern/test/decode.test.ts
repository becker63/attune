...
describe("runtime decode", () => {
    ...
    return Effect.runPromise(
      joern
        .query(
          raw(
            "cpg.x.toJson",
            Schema.Array(Schema.Struct({ x: Schema.Number })),
          ),
        )
        .pipe(
          Effect.tap((result) =>
            Effect.sync(() => expect(result).toStrictEqual([{ x: 1 }])),
          ),
        ),
    );
  });
  ...
  it("fails invalid JSON as JoernDecodeError", () => {
    ...
    return Effect.runPromise(
      Effect.exit(
        joern.query(raw("cpg.x.toJson", Schema.Array(Schema.Unknown))),
      ).pipe(
        Effect.tap((result) =>
          Effect.sync(() =>
            expect(String(result)).toContain(JoernDecodeError.name),
          ),
        ),
      ),
    );
