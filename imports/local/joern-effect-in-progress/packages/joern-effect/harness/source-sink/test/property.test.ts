...
const sourceSinkScenarioArbitrary = jsFixture.map(
  (fixture) =>
    new SourceSinkScenario({
      ...
      id: `source-sink-${fixture.functionName}-${fixture.sinkName}`,
...
const writeGeneratedJsRepo = async (
  source: string,
...
const canRunJoernE2E =
  envFlagEnabled(EnvVars.JoernEffectE2e) &&
  Boolean(readEnv(EnvVars.JoernBinary) ?? readEnv(EnvVars.JoernHome))
...
describe.skipIf(!canRunJoernE2E)("property-based Joern e2e behavior", () => {
  it(
  ...
  it(
    "generated JavaScript call sites round-trip through Joern rows",
    async () => {
      await fc.assert(
        fc.asyncProperty(jsFixture, async (fixture) => {
          const repo = await writeGeneratedJsRepo(
            [
  ...
  it(
    "generated JavaScript call sites materialize schema-edge evidence graphs",
    async () => {
      await fc.assert(
        fc.asyncProperty(jsFixture, async (fixture) => {
          const repo = await writeGeneratedJsRepo(
          ...
          try {
            ...
            expect(graph.nodes.some((node) => node.name === fixture.sinkName)).toBeTruthy()
            expect(graph.nodes.some((node) => node.name === fixture.functionName)).toBeTruthy()
            expect(graph.edges.every((edge) =>
              graph.nodes.some((node) => node.id === edge.source) &&
              graph.nodes.some((node) => node.id === edge.target),
            )).toBeTruthy()
  ...
  it(
    "generated JavaScript service shapes produce graph facts through the V2 surface",
    async () => {
      await fc.assert(
        fc.asyncProperty(jsFixture, async (fixture) => {
          const repo = await writeGeneratedJsRepo(
          ...
          try {
            ...
            expect((facts[0]?.evidence.graph as { readonly nodes?: readonly unknown[] }).nodes?.length).toBeGreaterThan(0)
          } finally {
            await rm(repo.dir, { force: true, recursive: true })
          }
        }),
