...
def _aggregate(arm_id: str, records: tuple[TrialRecord, ...]) -> Aggregate:
    ...
    total_cost = sum(record.metrics.cost_units for record in records)
    metrics = {
...
def _aggregates(runs: tuple[TrialRecord, ...]) -> tuple[Aggregate, ...]:
    grouped: dict[str, list[TrialRecord]] = {}
    for record in runs:
        grouped.setdefault(record.arm_id, []).append(record)
    return tuple(_aggregate(arm, tuple(rows)) for arm, rows in sorted(grouped.items()))


def _cost_leverage(
    aggregates: dict[str, Aggregate], numerator: str, denominator: str
) -> float | None:
    left, right = aggregates.get(numerator), aggregates.get(denominator)
...
def make_manifest(
    *,
    ...
    cases = tuple(sorted({record.case_id for record in runs}))
    ...
    return Manifest(
        ...
        comparisons={
            "C/A": {"cost_leverage": _cost_leverage(by_arm, "C", "A")},
            "B/A": {"cost_leverage": _cost_leverage(by_arm, "B", "A")},
            "C/B": {"cost_leverage": _cost_leverage(by_arm, "C", "B")},
        },
    )
