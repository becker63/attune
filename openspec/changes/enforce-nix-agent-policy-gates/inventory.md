...
### Initial Legacy Workflow

- Root scripts in `package.json` used package-manager and env-prefix command
  wrappers for normal Nx, build, check, generate, lint, property, architecture,
  and Codex commands before the policy ratchet.
...
- `scripts/codex/pnpm.mjs` previously had package-manager fallbacks outside the
  Nix toolchain substrate.
- `scripts/check-cloud-env.sh` previously combined package-manager bootstrap,
  install, graph, and typechecks.
- `scripts/codex/check-cloud-env.mjs` shells through
  `node scripts/codex/pnpm.mjs`.
- `scripts/architecture/ts-extended-diagnostics.mjs` shells to TypeScript for
  package-level extended diagnostics.
...
- Several package `project.json` files previously called package-local binary
  paths directly for typecheck targets.
...
- `AGENTS.md`, `docs/platform/codex-cloud-environment.md`,
  `docs/platform/autonomous-codex-workstation.md`, and
  `docs/linear/codex-pr-completion-gate.md` are the active agent-facing docs
  that must keep Nx as the public workflow surface.
...
- This OpenSpec change intentionally inventories forbidden surfaces in proposal,
  design, specs, tasks, and this inventory.
- Older OpenSpec changes may contain historical validation examples that still
  use package-manager bootstrap or env-prefix command wrappers.
