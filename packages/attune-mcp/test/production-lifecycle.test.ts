  ...
  it("replays a successful materialization through fresh service and MCP instances", async () => {
    await withTemporaryDirectory(
      ...
      async (home) => {
        ...
        const firstHandlers = makeMcpHandlers(
          makeInvestigationService(runtime),
        );
        expect(
          await Effect.runPromise(firstHandlers.repository_materialize(input)),
        ).toEqual(result);
        ...
        expect(
          await Effect.runPromise(
            restartedHandlers.repository_materialize(input),
          ),
        ).toEqual(result);
      },
    );
