...
export const createHomeDeploymentPlan = (
  ...
  const globalGateIds = ["tailscale-auth-ready", "k3s-token-ready"]
  ...
  for (const host of config.hosts) {
    ...
    for (const gate of gates) {
      const gateResource: PlannedResource = {
        id: gate.id,
        phase: "hosts",
        kind: "ManualGate",
    ...
    const k3sReadinessDependency = host.role === "k3s-init" ? `${host.hostname}:k3s-init` : `${host.hostname}:k3s-join`
    resources.push({
  ...
  resources.push(
    ...
    },
    {
      ...
      dependsOn: ["home-kubeconfig", ...config.hosts.map((host: ThinkCentreHost) => `${host.hostname}:k3s-readiness`)],
      ...
      ...(isCompleted(state, "attune-platform-kubernetes-graph")
        ? { status: "ready" as const }
        : blockedUnlessSatisfied(state, [
            "home-kubeconfig",
            ...config.hosts.map((host: ThinkCentreHost) => `${host.hostname}:k3s-readiness`),
          ])),
    },
    {
