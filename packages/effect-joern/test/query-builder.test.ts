...
describe("query builder", () => {
  ...
  it("escapes Scala strings", () => {
    const query = cpg.call
      .name('a "quoted" \\ name')
      .select({ code: prop.code });
    expect(query.cpgql).toContain('.name("a \\"quoted\\" \\\\ name")');
  });

  it("preserves regex flags supported by Joern's Java regex engine", () => {
    const query = cpg.call.name(/exec/im).select({ code: prop.code });
    expect(query.cpgql).toContain('.name("(?im)exec")');
  });
  ...
  it("combines curated steps with typed and raw escape hatches", () => {
