...
import type { NormalizedProperty, NormalizedSchema } from "./types.ts";
    ...
    writer.indent(() => {
      for (const property of schema.properties) {
        ...
        writer.indent(() => {
          ...
          writer.writeLine(
            `cardinality: ${JSON.stringify(property.cardinality)},`,
          );
          writer.writeLine(`owners: ${JSON.stringify(property.owners)},`);
          if (property.cpgName === "FILENAME") {
            writer.writeLine(
...
export const renderCpg = (schema: NormalizedSchema): string => {
  ...
  for (const node of schema.nodes) {
    starters.add(node.starterName);
  }
  for (const node of schema.nodes) {
    if (node.comment) {
      starterDocs.set(node.starterName, node.comment);
    }
  }
  file.addStatements((writer) => {
    writer.newLine();
