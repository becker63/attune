...
import { booleanField, field, isAstRecord } from "./_ast-record.js";

const isEffectProvide = (node: ESTree.Node): boolean => {
  if (node.type !== "CallExpression") return false;
  return Option.isSome(AST.matchCallOf(node, "Effect", "provide"));
};

const countProvides = (args: ReadonlyArray<ESTree.Node>): number => {
  let count = 0;
  for (const arg of args) {
    if (isEffectProvide(arg)) count++;
  }
  return count;
};
      ...
      CallExpression: (node) => {
        ...
        const isPipe =
          ...
          (callee.type === "MemberExpression" &&
            "property" in callee &&
            booleanField(callee, "computed") !== true &&
            (() => {
              const prop = field(callee, "property");
              return (
                isAstRecord(prop) && "name" in prop && prop["name"] === "pipe"
              );
            })());

        if (!isPipe) return Effect.void;

        if (countProvides(args) >= 2) {
          return ctx.report(
            Diagnostic.make({
              node,
              message:
