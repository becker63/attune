...
import type { TraversalSegment } from '../builder/traversal.js';
...
const includeNodeExpr = (include: GraphIncludeAst, index: number): string | undefined => {
  ...
  if (include.cpgql) {return `__rootNodes.flatMap(node => (${include.cpgql}).l).toList`}
  return undefined
}
...
Map(
  "nodes" -> __nodes.map(node => __jeNode(node, "${escapeScalaString(materialization.name)}")),
  "edges" -> __edges
).toJson`
}
