...
describe("framework policy CLI", () => {
  ...
  it("accepts authored legacy package facts without a generated semantic companion", () => {
    const workspaceRoot = makeWorkspace({
  ...
  it("warns when legacy package facts grow past the staged slim-file threshold", () => {
    const workspaceRoot = makeWorkspace({
  ...
  it("warns when package-local Attune companions remain beside the authored declaration", () => {
    const workspaceRoot = makeWorkspace({
      ...
      "packages/generated-companions/attune.artifact-ownership.json": JSON.stringify({ project: "generated-companions" }),
    })
  ...
  it("warns when a completed one-file root lacks replacement parity for package-local Attune companions", () => {
    const workspaceRoot = makeWorkspace({
      "packages/canopy/platform-alchemy-k8s/package.json": JSON.stringify({ name: "@attune/platform-alchemy-k8s" }),
      "packages/canopy/platform-alchemy-k8s/src/attune.package.ts": authoredProjectFactsSource({
        projectId: "platform-alchemy-k8s",
        projectKind: "platform-resource-provider",
      }),
      "packages/canopy/platform-alchemy-k8s/attune.artifact-ownership.json": JSON.stringify({ project: "platform-alchemy-k8s" }),
    })
    ...
    expect(result.ratchetDiagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        code: "package-local-attune-companion",
        filePath: "packages/canopy/platform-alchemy-k8s/src/attune.package.ts",
        severity: "warning",
      }),
    ]))
  })
  ...
  it("errors when completed project source imports a project-local generated artifact after replacement parity", () => {
    const workspaceRoot = makeWorkspace({
      ...
      "packages/canopy/platform-alchemy-k8s/src/local-contract.ts": "export { PackageContract } from \"./attune.contract.generated.js\"",
    })
    ...
    expect(result.ratchetDiagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        filePath: "packages/canopy/platform-alchemy-k8s/src/local-contract.ts",
        severity: "error",
      }),
    ]))
  })
  ...
  it("rejects stale generated files and manual derived ledger truth markers", () => {
    const workspaceRoot = makeWorkspace({
      ...
      "packages/generated-drift/src/generated/stale.generated.ts": [
        "// @generated by @attune/framework-nx",
  ...
  it("rejects old ontology runtime objects in primary program-index paths", () => {
    const workspaceRoot = makeWorkspace({
      ...
      "packages/trellis/runtime/src/ProgramIndexProjection.ts": [
...
function authoredProjectFactsSource(input: {
  ...
  return [
    "const defineAttuneLegacyPackageFacts = (facts) => facts",
