...
import type { AttuneOperationHandlers, AttuneTerminalLookups } from "../tools/registry.js";
import { repositoryCheckpoint, repositoryMaterialize } from "../tools/repository/implementation.js";

/**
 * Constructs runtime handlers and terminal lookups over one workspace store.
 *
 * @remarks
 *   Shared engine and store identity preserve correlation across every registered operation. @param config -
 *   Runtime configuration. @returns Handlers, terminal lookups, and their workspace store.
 */
export const makeAttuneRuntime = (
  ...
  const handlers: AttuneOperationHandlers = {
    ...
    repository_checkpoint: (input) => repositoryCheckpoint(engine, workspaces, input),
    ...
    investigation_finalize: (input) => investigationFinalize(engine, workspaces, input),
  };
  const terminalLookups: AttuneTerminalLookups = {
    repository_checkpoint: (input) => engine.lookupTerminal("repository_checkpoint", input),
    ...
    artifact_promote: (input) => engine.lookupTerminal("artifact_promote", input),
    investigation_finalize: (input) => engine.lookupTerminal("investigation_finalize", input),
  };
