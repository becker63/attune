...
import {
  advanceFixtureStep,
  decodeFixtureStepResult,
  replayFixtureRouteEvents,
  resetFixtureRouteRuntimeForTest,
  startFixtureRoute,
} from "../src/fixture-route.js";
import { AdvanceFixtureStep, StartFixtureRun } from "../src/fixture-commands.js";
import { attuneFoldkitSiteFixture } from "../src/fixtures/app-site-fixture.js";
...
describe("React-to-FoldKit migration story contract", () => {
  ...
  test("activity filters are behavior, not React component state", () => {
    Story.story(
      update,
      Story.with(initialModel()),
  ...
  test("site fixture declares deterministic coverage for every existing route surface", () => {
    expect(attuneFoldkitSiteFixture.routes).toEqual(
    ...
    expect(
      attuneFoldkitSiteFixture.surfaces.map((surface) => surface.surfaceId),
    ).toEqual(
      expect.arrayContaining([
        "workbench",
        "discover",
    ...
    expect(attuneFoldkitSiteFixture.items.length).toBeGreaterThan(0);
  });
  ...
  test("fixture route event decoding and replay stay deterministic", async () => {
    resetFixtureRouteRuntimeForTest();
    const startResult = await startFixtureRoute();
    const anchorId = "anchor-effect-reactivity-dag";
    const selectedResult = await advanceFixtureStep("select-anchor", {
      selectedAnchorId: anchorId,
    });

    expect(decodeFixtureStepResult(startResult).summary.eventCount).toBe(3);
    expect(
      selectedResult.trace.at(-1)?.semanticEventTags,
    ).toEqual([]);
    expect(
      selectedResult.routeEvents.at(-1)?.targetId,
    ).toBe(anchorId);
    expect(
      replayFixtureRouteEvents(selectedResult.routeEvents).selectedAnchorId,
    ).toBe(anchorId);
  });

  test("anchor selection is FoldKit interaction state plus a typed command", async () => {
    resetFixtureRouteRuntimeForTest();
    const [model] = init();
    const command = AdvanceFixtureStep({
      selectedAnchorId: "anchor-effect-reactivity-dag",
      step: "select-anchor",
    });
    const result = await advanceFixtureStep("select-anchor", {
      selectedAnchorId: "anchor-effect-reactivity-dag",
    });
    ...
    Story.story(
      ...
      Story.with(model),
      Story.message(
        SelectedFixtureAnchor({ anchorId: "anchor-effect-reactivity-dag" }),
      ),
      Story.model((selectedModel) => {
        expect(selectedModel.fixtureRoute.selectedAnchorId).toBe(
          "anchor-effect-reactivity-dag",
        );
      }),
      Story.Command.expectExact(command),
      Story.Command.resolve(command, FixtureStepApplied({ result })),
      Story.Command.expectNone(),
    );
  });
});
