...
const isEffectMap = (node: ESTree.Node): boolean => {
  ...
  return Option.isSome(AST.matchCallOf(node, "Effect", "map"));
};

const isEffectFlatMap = (node: ESTree.Node): boolean => {
  if (node.type !== "CallExpression") return false;
  return Option.isSome(AST.matchCallOf(node, "Effect", "flatMap"));
};
    ...
    return {
      CallExpression: (node) => {
        ...
        if (calleeNode.type === "MemberExpression") {
          const prop = field(calleeNode, "property");
          if (
            isAstRecord(prop) &&
            "name" in prop &&
            prop["name"] === "pipe" &&
            hasMapFlatMapSequence(args)
          ) {
            return ctx.report(
              Diagnostic.make({
                node,
