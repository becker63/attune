...
import {
  FrameworkSqliteQuarantineReason,
  FrameworkSqliteQuarantineRecipes,
  ProgramFactStore,
...
describe("@attune/framework-sqlite", () => {
  it("declares SQLite only as a legacy quarantine recipe", () => {
    const records = FrameworkSqliteQuarantineRecipes.map((recipe) => RecipeRecordView.fromRecipe(recipe))

    expect(records).toEqual([
      expect.objectContaining({
        recipeId: "framework-sqlite.legacy-quarantine",
        kind: "recipe",
        sourcePath: "framework/sqlite/src/recipes.ts",
      }),
    ])
    expect(FrameworkSqliteQuarantineReason).toContain("legacy quarantine")
    expect(FrameworkSqliteQuarantineRecipes[0]?.title).toContain("Quarantine")
  })
