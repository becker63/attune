...
export const cpg = {
   ...
   * ANNOTATION node. Start a Joern traversal at `cpg.annotation`.
   */
  annotation: starter("annotation"),
  /**
   * A literal value assigned to an ANNOTATION_PARAMETER Start a Joern traversal
   * at `cpg.annotationLiteral`.
   */
  annotationLiteral: starter("annotationLiteral"),
  /**
   * Formal annotation parameter Start a Joern traversal at
   * `cpg.annotationParameter`.
   */
  annotationParameter: starter("annotationParameter"),
  /**
   * Assignment of annotation argument to annotation parameter Start a Joern
   * traversal at `cpg.annotationParameterAssign`.
   */
  annotationParameterAssign: starter("annotationParameterAssign"),
  /**
   * Initialization construct for arrays Start a Joern traversal at
   * `cpg.arrayInitializer`.
   */
   ...
   * The `REF` edge if present has priority. Start a Joern traversal at
   * `cpg.binding`.
   */
   ...
   * for "block expression") to describe what in the CPG we call "Block". Start
   * a Joern traversal at `cpg.block`.
   */
  block: starter("block"),
  /**
   ...
   * arguments and return value. Start a Joern traversal at `cpg.call`.
   */
  call: starter("call"),
  /**
   * Represents the binding of a LOCAL or METHOD_PARAMETER_IN into the closure
   * of a method Start a Joern traversal at `cpg.closureBinding`.
   */
  closureBinding: starter("closureBinding"),
  /**
   * A source code comment Start a Joern traversal at `cpg.comment`.
   */
   ...
   * corresponding configuration files. Start a Joern traversal at
   * `cpg.configFile`.
   */
  configFile: starter("configFile"),
  /**
   * This node represents a control structure as introduced by control structure
   * statements as well as conditional and unconditional jumps. Its type is
   * stored in the `CONTROL_STRUCTURE_TYPE` field to be one of several
   * pre-defined types. These types are used in the construction of the control
   * flow layer, making it possible to generate the control flow layer from the
   * abstract syntax tree layer automatically.
   *
   ...
   * is not relevant for construction of the control flow layer. Start a Joern
   * traversal at `cpg.controlStructure`.
   */
  controlStructure: starter("controlStructure"),
  /**
   * This node represents a dependency Start a Joern traversal at
   * `cpg.dependency`.
   */
   ...
   * they use the same expression to reference it. Start a Joern traversal at
   * `cpg.fieldIdentifier`.
   */
   ...
   * all elements of the code by file.
   *
   * For each file, the graph CAN contain exactly one File node, if not File
   * nodes are created as indicated by `FILENAME` property of other nodes. As
   * file nodes are root nodes of abstract syntax tress, they are AstNodes and
   * their order field is set to 0. This is because they have no sibling nodes,
   * not because they are the first node of the AST. Start a Joern traversal at
   * `cpg.file`.
   */
  file: starter("file"),
  /**
   ...
   * as well as a list of nodes that serve as evidence for the finding. Start a
   * Joern traversal at `cpg.finding`.
   */
  finding: starter("finding"),
  /**
   * This node represents an identifier as used when referring to a variable by
   * name. It holds the identifier's name in the `NAME` field and its
   * fully-qualified type name in `TYPE_FULL_NAME`. Start a Joern traversal at
   * `cpg.identifier`.
   */
  identifier: starter("identifier"),
  /**
   * Declarative import as it is found in statically typed languages like Java.
   * This kind of node is not supposed to be used for imports in dynamically
   * typed languages like Javascript. Start a Joern traversal at `cpg.import`.
   */
   ...
   * that this jump label is created from, e.g., "Label". Start a Joern
   * traversal at `cpg.jumpLabel`.
   */
   ...
   * construct that this jump target is created from, e.g., "Label". Start a
   * Joern traversal at `cpg.jumpTarget`.
   */
  jumpTarget: starter("jumpTarget"),
  /**
   * This node represents a key value pair, where both the key and the value are
   * strings. Start a Joern traversal at `cpg.keyValuePair`.
   */
   ...
   * type name, e.g., `java.lang.Integer`. Start a Joern traversal at
   * `cpg.literal`.
   */
   ...
   * initialization, e.g., for `int x = 10;`, it contains `int x`. Start a Joern
   * traversal at `cpg.local`.
   */
  local: starter("local"),
  /**
   ...
   * of the variable `i`. Start a Joern traversal at `cpg.member`.
   */
   ...
   * `OVERLAYS` specifies which overlays have been applied to the CPG. Start a
   * Joern traversal at `cpg.metaData`.
   */
   ...
   * source program.
   *
   * The `FULL_NAME` field specifies the method's fully-qualified name,
   * including information about the namespace it is contained in if applicable,
   * the name field is the function's short name. The field `IS_EXTERNAL`
   * indicates whether it was possible to identify a method body for the method.
   * This is true for methods that are defined in the source program, and false
   * for methods that are dynamically linked to the program, that is, methods
   * that exist in an external dependency.
   *
   * Line and column number information is specified in the optional fields
   * `LINE_NUMBER`, `COLUMN_NUMBER`, `LINE_NUMBER_END`, and `COLUMN_NUMBER_END`
   * and the name of the source file is specified in `FILENAME`. An optional
   * hash value MAY be calculated over the function contents and included in the
   * `HASH` field.
   *
   ...
   * `METHOD`, `TYPE_DECL` or `NAMESPACE_BLOCK`. Start a Joern traversal at
   * `cpg.method`.
   */
   ...
   * type name. Start a Joern traversal at `cpg.methodParameterIn`.
   */
   ...
   * are automatically created upon first loading the CPG. Start a Joern
   * traversal at `cpg.methodParameterOut`.
   */
   ...
   * method and the `TYPE_FULL_NAME` holds its fully-qualified type name. Start
   * a Joern traversal at `cpg.methodRef`.
   */
   ...
   * creators MUST NOT depend on this value. Start a Joern traversal at
   * `cpg.methodReturn`.
   */
   ...
   * argument in function calls. Start a Joern traversal at `cpg.modifier`.
   */
   ...
   *
   * NAMESPACE nodes MUST NOT be created by language frontends. Instead, they
   * are generated from NAMESPACE_BLOCK nodes automatically upon first loading
   * of the CPG. Start a Joern traversal at `cpg.namespace`.
   */
   ...
   * via a `package` statement in Java or a `namespace{ }` statement in C++.
   *
   * The `FULL_NAME` field contains a unique identifier to represent the
   * namespace block itself not just the namespace it references. So in addition
   * to the namespace name it can be useful to use the containing file name to
   * derive a unique identifier.
   *
   * The `NAME` field contains the namespace name in a human-readable format.
   * The name should be given in dot-separated form where a dot indicates that
   * the right hand side is a sub namespace of the left hand side, e.g.,
   ...
   * are represented via `METHOD_RETURN` nodes. Start a Joern traversal at
   * `cpg.return`.
   */
  return: starter("return"),
  /**
   * This node represents a tag. Start a Joern traversal at `cpg.tag`.
   */
  tag: starter("tag"),
  /**
   * This node contains an arbitrary node and an associated tag node. Start a
   * Joern traversal at `cpg.tagNodePair`.
   */
   ...
   * a type declaration. Start a Joern traversal at `cpg.type`.
   */
  type: starter("type"),
  /**
   ...
   * `CODE` field. Start a Joern traversal at `cpg.typeArgument`.
   */
   ...
   * `List[Integer]` where `Integer` is a concrete type.
   *
   * The language frontend MUST create type declarations for all types declared
   * in the source program and MAY provide type declarations for types that are
   * not declared but referenced by the source program. If a declaration is
   * present in the source program, the field `IS_EXTERNAL` is set to `false`.
   * Otherwise, it is set to `true`.
   *
   * The `FULL_NAME` field specifies the type's fully-qualified name, including
   * information about the namespace it is contained in if applicable, the name
   * field is the type's short name. Line and column number information is
   * specified in the optional fields `LINE_NUMBER`, `COLUMN_NUMBER`,
   * `LINE_NUMBER_END`, and `COLUMN_NUMBER_END` and the name of the source file
   * is specified in `FILENAME`.
   *
   * Base types can be specified via the `INHERITS_FROM_TYPE_FULL_NAME` list,
   * where each entry contains the fully-qualified name of a base type. If the
   * type is known to be an alias of another type (as for example introduced via
   * the C `typedef` statement), the name of the alias is stored in
   * `ALIAS_TYPE_FULL_NAME`.
   *
   * Finally, the fully qualified name of the program constructs that the type
   * declaration is immediately contained in is stored in the
   * `AST_PARENT_FULL_NAME` field and its type is indicated in the
   * `AST_PARENT_TYPE` field to be one of `METHOD`, `TYPE_DECL` or
   * `NAMESPACE_BLOCK`. Start a Joern traversal at `cpg.typeDecl`.
   */
   ...
   * carries only a `NAME` field that holds the parameters name. Start a Joern
   * traversal at `cpg.typeParameter`.
   */
  typeParameter: starter("typeParameter"),
  /**
   * Reference to a type/class Start a Joern traversal at `cpg.typeRef`.
   */
   ...
   * included using a node of type `UNKNOWN`. Start a Joern traversal at
   * `cpg.unknown`.
   */
