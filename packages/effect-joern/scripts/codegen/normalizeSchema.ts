...
const camel = (value: string): string =>
  value
    .toLowerCase()
    .replace(/[_\s-]+(?<next>[a-z0-9])/gu, (_, char: string) =>
      char.toUpperCase(),
    );
...
const mergeProperties = (
  ...
  Object.values(
    properties
      ...
      .reduce<Readonly<Record<string, NormalizedProperty>>>(
...
const normalizeNode = (
  ...
  const properties = nodeProperties
    .filter(
      (rawProperty): rawProperty is RawProperty =>
        typeof rawProperty !== "string",
    )
...
export const normalizeSchema = (raw: RawSchema): NormalizedSchema => {
  ...
  const rawProperties = (raw.properties ?? []).map((property) =>
