...
The checked-in schema snapshot is `schema/joern-cpg-schema.{{CPG_VERSION}}.json`. It is extracted by compiling `scripts/ExtractCpgSchema.java` against the Nix-built Joern jars and reading Joern's real `GraphSchema` metadata.
