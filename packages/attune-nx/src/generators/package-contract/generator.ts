...
const typecheckSource = (operationId: string): string => `import type {
  AssertExactHandlers,
...
type PackageFuzzHandlers = {
  readonly ${literal(operationId)}: () => unknown
}

type PackageProperties = {
  readonly ${literal(operationId)}: () => boolean
}
...
export default function packageContractGenerator(
  ...
  writeTextIfChanged(tree, typecheckPath, typecheckSource(model.operationId))
  ...
  upsertSourceBom(tree, {
    ...
    ownedFiles: [contractPath, typecheckPath],
    editableRegions: [
      {
        file: contractPath,
