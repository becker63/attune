...
- Active product/proof/platform packages under `packages/`: `attune-nx`,
...
- Planned root framework projects: `framework/protocol`, `framework/runtime`,
  `framework/sqlite`, `framework/language-service`, `framework/nx`,
  `framework/testing`, `framework/architecture`, and
  `framework/oxlint-policy`. These may be Nx/pnpm workspace projects, but they
  are framework internals/substrate, not product packages under `packages/`.
- Final architecture project identity: `attune-architecture`, with final path
  `framework/architecture`. The old architecture-lint identity is only a
...
- Generator/tooling packages: `attune-nx` and generator surfaces in
  `attune-pi-agent`.
- Policy framework projects: `framework/architecture` with Nx id
  `attune-architecture`, and `framework/oxlint-policy` with Nx id
...
### `framework/architecture` (`attune-architecture`, formerly `attune-architecture-lint`)

Current role:
- Architecture policy scanner for workflow surfaces, Source BOM ownership, and
  generator shape conformance.
- Broader framework architecture policy project, not a product package and not
  only lint.
...
Target contract:
- Final framework path: `framework/architecture`.
- Nx project id and CLI bin: `attune-architecture`.
...
- Source BOM contract shard: generated in
  `framework/architecture/attune.source-bom.json`, generated by
  `@attune/nx:package-contract` through `sync-package-contract`.
- Owning implementation agent: `attune-architecture-migration-agent`.
- Ledger agent status: `framework/architecture/src/attune.package.ts`
  and `framework/architecture/src/attune.package.typecheck.ts` are
  present and tracked by generator-shape conformance as generated package
  contract outputs. The archive-hardening pass moved the project out of
  `packages/` and into the root framework layer.
...
- Keep the package-contract kernel in `framework/protocol`; architecture policy
  consumes `@attune/framework-protocol` and no longer owns the type kernel.
...
### `framework/oxlint-policy` (`effect-oxlint-policy`)
...
Target contract:
- Final framework path: `framework/oxlint-policy`.
- Nx project id: `effect-oxlint-policy`.
- Package name: `@attune/framework-oxlint-policy`.
...
- Source BOM contract shard: generated in
  `framework/oxlint-policy/attune.source-bom.json`, generated by
  `@attune/nx:package-contract` through `sync-package-contract`.
- Owning implementation agent: `effect-oxlint-policy-migration-agent`.
- Ledger agent status:
  `framework/oxlint-policy/src/attune.package.ts` and
  `framework/oxlint-policy/src/attune.package.typecheck.ts` are present,
  with `framework/oxlint-policy/test/attune-package-contract.test.ts`,
...
- Stale-output checks now prove generated contract and materialization files
  match framework/Nx output where required by build/typecheck.
- The plugin has moved out of `packages/`; oxlint loads it from
