...
Attune SHALL persist only non-secret schemas, public recipient metadata, templates, and source code in the repository while keeping encrypted runtime secret files, local deployment state, and secret-bearing evidence outside git.
