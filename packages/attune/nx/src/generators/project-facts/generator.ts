...
const symbolMetadata = (model: ProjectFactsGeneratorModel): string => {
  ...
  switch (model.symbolKind) {
    ...
    case "generator":
      return `      observes: [\n        {\n          id: ${literal("@attune/nx:project-facts")},\n          kind: "recipe-receipt-provenance" as const,\n        },\n      ],`
    case "joern-template":
...
const sourceRootFromDirectory = (
  ...
  if (explicitSourceRoot !== undefined) return explicitSourceRoot
  const segments = directory.split("/").filter(Boolean)
  const sourceIndex = segments.indexOf("src")
  return sourceIndex > 0 ? segments.slice(0, sourceIndex).join("/") : "."
}
 ...
 * truth. Runtime/cache rows live under framework-owned recipe receipt storage.
 */
