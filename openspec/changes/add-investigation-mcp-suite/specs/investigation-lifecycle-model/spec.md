...
### Requirement: Direct lifecycle modules
...
finalization lifecycle behavior. Each supported tool SHALL have one visible
implementation under `tools`, and the MCP server SHALL remain an adapter over
the application service. The tree SHALL NOT add per-tool descriptor files,
public constants, or barrel modules when the closed registry already carries
that relationship.
...
- **THEN** the contributor finds `tools/maude/implementation.ts` and can follow
  its keyed invocation into `investigation` without first discovering the MCP
  registration implementation

### Requirement: Small public onboarding surface

The supported `attune-mcp` package entry SHALL expose at most twenty named
exports. Its operational authorities SHALL be one `AttuneToolkit`, one
`ATTUNE_OPERATIONS` registry, and one `InvestigationService`. The remaining
exports MAY be the service factory, keyed operation projections, the three
state capabilities, and caller-visible failures.

The package entry SHALL NOT export per-tool operation constants, implementation
handlers, handler compatibility maps, persistence stores, invocation engines,
platform helpers, or recursive module barrels. Those remain source-level
...
- **THEN** it observes at most twenty named exports
- **AND** the contributor can reach the complete supported model through the
  Toolkit, registry, service, keyed projections, state capabilities, and
  failures without choosing among duplicate descriptor or handler concepts
...
The keyed Toolkit result and error projection SHALL be runtime-validated before
