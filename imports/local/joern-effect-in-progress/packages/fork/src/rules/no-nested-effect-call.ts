...
const getEffectMethodName = (node: unknown): string | undefined => {
  if (!isAstRecord(node) || node["type"] !== "CallExpression") return undefined;
  ...
  if (!isAstRecord(obj) || !isAstRecord(prop)) return undefined;
  if (obj["type"] !== "Identifier" || obj["name"] !== "Effect") return undefined;
  if (prop["type"] !== "Identifier" || typeof prop["name"] !== "string") return undefined;
  return prop["name"];
};
