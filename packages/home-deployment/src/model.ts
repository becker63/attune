...
const host = (index: 1 | 2 | 3): ThinkCentreHost => {
  ...
  return {
    ...
    sopsRecipient: {
      ...
      publicKeyPath: `nix/hosts/sops-recipients/hosts/${hostname}.age`,
...
export const defaultHomeDeploymentConfig = (): HomeDeploymentConfig => ({
  ...
  operator: {
    ...
    sops: {
      sopsFile: ".sops.yaml",
      recipientMetadataFile: "nix/hosts/sops-recipients/recipients.json",
...
const sopsRecipientRotationCommand = (config: HomeDeploymentConfig, hostConfig: ThinkCentreHost): CommandPlan => {
  ...
  const recipientDirectory = hostConfig.sopsRecipient.publicKeyPath.replace(/\/[^/]+$/, "")
  ...
  return commandPlan([
    ...
    [
      ...
      `recipients="$( { test -r "$operator_age_key" && sed -n 's/^# public key: //p' "$operator_age_key" | head -n 1; cat ${shellQuote(recipientDirectory)}/*.age; } | awk 'NF' | sort -u | paste -sd, -)"`,
      `test -n "$recipients"`,
