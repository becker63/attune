...
): string => `${generatedHeader(recipeId, projection, sourceDescriptor)}${source}`
  ...
  Effect.gen(function* emitFastCheckArbitrariesEffect() {
    ...
    yield* Effect.tryPromise({
      ...
      try: () =>
        format(
          withGeneratedHeader(
            renderFastCheckArbitraries(schema),
            "joern-effect.generated-fast-check-arbitraries",
            "joern-effect.codegen.fast-check-arbitraries",
            "packages/attune/joern-effect/schema/joern-cpg-schema.1.7.70.json",
          ),
          { parser: "typescript" },
        ).then(
