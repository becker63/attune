...
- Active packages: `attune-nx`, `attune-architecture-lint` currently becoming
  final package id `attune-architecture`,
...
- Final architecture package identity: `attune-architecture`, with final path
  `packages/attune-architecture`. `attune-architecture-lint` is only the
  current pre-rename identity and must not remain in final docs, ledgers,
...
### `attune-architecture-lint` -> `attune-architecture`
...
- CLI bin remains named `attune-architecture-lint`.
...
- Source BOM contract shard: generated in
  `packages/attune-architecture-lint/attune.source-bom.json`, generated by
  `@attune/nx:package-contract` through `sync-package-contract`.
- Owning implementation agent: `attune-architecture-migration-agent`.
- Ledger agent status: `packages/attune-architecture-lint/src/attune.package.ts`
  and `packages/attune-architecture-lint/src/attune.package.typecheck.ts` are
...
Explicit remaining Phase 4 debt:
- Finish the physical package path/bin/docs rename from
  `attune-architecture-lint` to `attune-architecture`.
...
Final cleanup:
- Rename package, project, bin, docs, generated ledger owner, and public
  surfaces to `attune-architecture`.
- Treat `attune-architecture-lint` as a migration note only after the rename.
